...
public class CloudFoundryOperationsBeanProcessor {
    ...
    public void process(){
        ...
        cloudFoundryProperties.getOrganizations()
                .forEach( (datacenter , orgs) -> {

                    if(logger.isInfoEnabled()) { logger.info("orgs {} datacenter {}",orgs,datacenter); }
                    orgs.forEach( org -> {


                        List<String> spaces = cloudFoundryProperties.getSpaces().get(org);
